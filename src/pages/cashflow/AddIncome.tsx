...
export default function AddIncome() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await addIncome({ name: name.trim(), amount: parseFloat(amount), category, notes: notes.trim() });
      toast.success("Income added successfully!", { duration: NUM.TOAST_SUCCESS_DURATION });
      setTimeout(() => toast.info("Money status updated", { duration: NUM.TOAST_DURATION }), 500);
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl shadow-card p-5 space-y-5">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Income Name *</label>
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Salary"
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? <><i className="bx bx-loader-alt bx-spin" /> Adding...</> : <><i className="bx bx-plus" /> Add Income</>}
        </button>
      </form>
    </div>
  );
