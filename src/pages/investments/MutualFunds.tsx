...
export default function MutualFunds() {
  ...
  const handleUpdate = async () => {
    ...
    setSubmitting(true);
    try {
      const updates: Partial<MutualFund> = {
        lastUpdated: new Date().toISOString(),
      };
      if (updateForm.investedAmount)
        updates.investedAmount =
          selectedFund.investedAmount + parseFloat(updateForm.investedAmount);
      if (updateForm.currentValue)
        updates.currentValue = parseFloat(updateForm.currentValue);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        action={
          <button
            onClick={() => navigate(ROUTES.INVESTMENTS_FUNDS_NEW)}
          ...
          >
            <i className="bx bx-plus" /> New Fund
          </button>
        }
      />
      <div className="">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          icon="bx-trending-up"
          variant={totalValue >= totalInvested ? "success" : "destructive"}
          trend={totalValue >= totalInvested ? "up" : "down"}
          trendValue={`${totalGrowth}%`}
        />
        <StatCard
          title="Active Funds"
          value={`${funds.length}`}
      ...
      {sortedFunds.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  SIP: {formatCurrency(fund.sipAmount)}/mo · Updated{" "}
                  {formatDate(fund.lastUpdated)}
                </p>
                <div className="flex gap-2">
                  <button
                    onClick={() => openUpdate(fund)}
                    className="flex-1 text-xs font-medium py-2 rounded-lg bg-accent text-accent-foreground hover:bg-primary/20 transition-colors"
                  >
                    Update
                  </button>
                  <button
                    onClick={() => handleDelete(fund.id)}
                    className="text-xs font-medium py-2 px-3 rounded-lg bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors"
                  >
                    Delete
                  </button>
                </div>
        ...
        <EmptyState
          icon="bx-bar-chart-alt-2"
          title="No funds yet"
          description="Add your first mutual fund"
          action={
            <button
              onClick={() => navigate(ROUTES.INVESTMENTS_FUNDS_NEW)}
              className="gradient-primary text-primary-foreground text-sm font-medium px-4 py-2 rounded-lg"
            >
              Add Fund
            </button>
          }
      ...
      <Modal
        ...
        title={`Update: ${selectedFund?.name || ""}`}
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              type="number"
              value={updateForm.investedAmount}
              onChange={(e) =>
                setUpdateForm({ ...updateForm, investedAmount: e.target.value })
              }
              placeholder="0.00"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setUpdateForm({ ...updateForm, currentValue: e.target.value })
              }
              placeholder={selectedFund?.currentValue?.toString()}
              className="w-full px-3 py-2.5 rounded-lg bg-secondary border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          <div>
          ...
          >
            {submitting ? (
              ...
              <>
                <i className="bx bx-check" /> Update Fund
              </>
            )}
          </button>
        </div>
