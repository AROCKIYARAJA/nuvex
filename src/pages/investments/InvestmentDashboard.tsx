...
export default function InvestmentDashboard() {
  ...
  const trendData = {
    ...
    datasets: [{
      ...
      data: [summary?.totalValue * 0.85 || 0, summary?.totalValue * 0.9 || 0, summary?.totalValue * 0.95 || 0, summary?.totalValue || 0],
    ...
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate(ROUTES.INVESTMENTS_METALS)} className="bg-warning text-warning-foreground text-sm font-medium px-4 py-1.5 rounded-lg hover:opacity-90 transition-opacity flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-4 shadow-card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display font-semibold text-foreground">Portfolio Growth</h3>
          ...
          <div className="h-64">
            <Line data={trendData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { color: resolvedTheme === "dark" ? "#64748b" : "#94a3b8" } }, y: { grid: { color: resolvedTheme === "dark" ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)" }, ticks: { color: resolvedTheme === "dark" ? "#64748b" : "#94a3b8" } } } }} />
          </div>
        </div>

        <div className="bg-card border border-border rounded-xl p-4 shadow-card">
          <h3 className="font-display font-semibold text-foreground mb-4">Allocation</h3>
          <div className="h-48 flex items-center justify-center">
            <Doughnut data={allocationData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom", labels: { color: resolvedTheme === "dark" ? "#94a3b8" : "#64748b", padding: 16 } } }, cutout: "65%" }} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-card">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Add Gold", icon: "bx-coin", path: ROUTES.INVESTMENTS_METALS, color: "bg-warning/10 text-warning" },
            { label: "Add Fund", icon: "bx-plus-circle", path: ROUTES.INVESTMENTS_FUNDS_NEW, color: "bg-primary/10 text-primary" },
            { label: "View Metals", icon: "bx-bar-chart", path: ROUTES.INVESTMENTS_METALS, color: "bg-accent text-accent-foreground" },
            { label: "View Funds", icon: "bx-list-ul", path: ROUTES.INVESTMENTS_FUNDS, color: "bg-success/10 text-success" },
          ].map((a) => (
            <button key={a.label} onClick={() => navigate(a.path)} className={cn("flex items-center gap-2 p-3 rounded-xl border border-border hover:shadow-card transition-all text-sm font-medium", a.color)}>
